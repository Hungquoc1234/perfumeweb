<?php
    ...
    $id = $imageName = $imagePath = $name = $price = $supplier = $sex = $scent = $concertration = $description = '';
    ...
    if(isset($_GET['id'])){
        ...
        if(mysqli_num_rows($result) > 0){
            ...
            $sex = $row['sex'];
            $scent = $row['scent'];
            $concertration = $row['concertration'];
    ...
    $check = mysqli_query($con, "UPDATE product SET name = '$name', image = '$imageName', price = $price, supplier = '$supplier', sex = '$sex', scent = '$scent', concertration = '$concertration', description = '$description' WHERE id = '$id'");
        // mysqli_query($con, "insert into product values('$id', '$name', '$image', '$price', '$supplier', '$sex', '$scent', '$concertration', '$description', '$srt', '$sre')");
        ...
        <div class="body">
            <div class="container">
                <form method="post" enctype="multipart/form-data">
                    ...
                    <img src="<?php echo '..\img\\'.$imageName?>" id="uploadedImage"><br>
                    ...
                    <input type="text" value="<?php echo $supplier ?>" name="supplier" ><br>

                    <label for="sex">Sex:</label>
                    <select name="sex" id="sex">
                        <?php
                            $sexOption = ['male', 'female', 'unisex'];

                            foreach($sexOption as $i){
                                if($i == $sex){
                                    echo '<option value="'.$i.'" selected>'.ucfirst($i).'</option>';
                                } else{
                                    echo '<option value="'.$i.'">'.ucfirst($i).'</option>';
                                }
                            }
                        ?>
                    </select><br>
                    ...
                    <select name="scent" id="scent">
                        <?php
                            $scentOption = ['floral', 'citrust', 'green', 'aquatic', 'woody'];

                            foreach($scentOption as $i){
                                if($i == $scent){
                                    echo '<option value="'.$i.'" selected>'.ucfirst($i).'</option>';
                                } else{
                                    echo '<option value="'.$i.'">'.ucfirst($i).'</option>';
                    ...
                    <select name="concertration" id="concertration">
                        <?php
                            $concertrationOption = ['perfume', 'edp', 'edt', 'edc'];

                            foreach($concertrationOption as $i){
                                if($i == $concertration){
                                    echo '<option value="'.$i.'" selected>'.ucfirst($i).'</option>';
                                } else{
                                    echo '<option value="'.$i.'">'.ucfirst($i).'</option>';
